...
class Xcom_Ebay_Block_Adminhtml_Widget_Grid_Column_Renderer_Action
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Action
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        if ($row instanceof Xcom_Mmp_Model_Policy) {
            foreach ($actions as $action){
                if (is_array($action)) {
                    if ($row->getStatus() == Xcom_Mmp_Model_Policy::XML_POLICY_STATUS_FAILED) {
                        $action = array(
                            'caption' => Mage::helper('xcom_ebay')->__('Failed'),
                        );
                        return $this->_toTextHtml($action, $row);
                    }
                    else {
