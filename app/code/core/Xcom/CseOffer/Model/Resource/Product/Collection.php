...
class Xcom_CseOffer_Model_Resource_Product_Collection extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Collection
{
    /**
     * Processing collection items after loading.
     * Modify channels, cseoffer fields.
     *
     * @return Xcom_CseOffer_Model_Resource_Product_Collection
     */
    ...
    /**
     * Get cseoffers column
     *
     * @return Zend_Db_Expr
     */
    protected function _getCseOfferStatusIdsColumn()
    {
        return new Zend_Db_Expr('GROUP_CONCAT(('
            . ' CASE'
                . ' WHEN ccp.channel_product_id IS NOT NULL THEN ccp.offer_status'
                . ' ELSE NULL'
    ...
    public function joinStatusColumn($channelTypeCode)
    {
        $this->getSelect()
            ->joinLeft(
                array('ccp' => $this->getTable('xcom_cseoffer/channel_product')), 'ccp.product_id = e.entity_id', array())
            ->joinLeft(array('xcc' => $this->getTable('xcom_cse/channel')),
            ...
            ->columns(array('ccp.channel_product_id', 'xcc.channel_id',
                'cseoffer'       => new Zend_Db_Expr($this->_getCseOfferStatusIdsColumn()),
    ...
    public function joinChannelData($channelId)
    {
        ...
        $channelOfferStatus = new Zend_Db_Expr('CASE'
                . ' WHEN xccp.offer_status = ' . Xcom_CseOffer_Model_Channel_Product::STATUS_ACTIVE
                . ' THEN \'' . $helper->__('Active') .'\''
                . ' WHEN xccp.offer_status = ' . Xcom_CseOffer_Model_Channel_Product::STATUS_INACTIVE
                . ' THEN \'' . $helper->__('Inactive') .'\''
                . ' WHEN xccp.offer_status = ' . Xcom_CseOffer_Model_Channel_Product::STATUS_FAILURE
                . ' THEN \'' . $helper->__('Failure') .'\''
                . ' WHEN xccp.offer_status = ' . Xcom_CseOffer_Model_Channel_Product::STATUS_PENDING
                . ' THEN \'' . $helper->__('Pending') .'\''
            . ' END'
        );
        $this->getSelect()
            ->joinLeft(array('xccp' => $this->getTable('xcom_cseoffer/channel_product')),
                'xccp.product_id = e.entity_id AND xccp.channel_id  = :channel_id')
            ->columns(array('channel_offer_status' => $channelOfferStatus));
    ...
    /**
     * Prepare filter for custom column offers
     *
     * @param  $condition
     * @return Xcom_CseOffer_Model_Resource_Product_Collection
     */
    public function prepareCseOffersFilter($condition)
    {
        if (isset($condition['eq']) && !is_array($condition['eq'])) {
            $this->_addHavingColumn('channel_id', $condition['eq'], $this->_getCseOfferIdsColumn());
        }
        return $this;
    }
