 ...
 */
class Xcom_Mmp_Block_Adminhtml_Widget_Grid_Massaction
    extends Mage_Adminhtml_Block_Widget_Grid_Massaction
{
    ...
    /**
     * Customized getGridIdsJson() method.
     * Added exception for Policies collection.
     *
     * @return string
     */
    public function getGridIdsJson()
    {
        $firstItem = $this->getParentBlock()->getCollection()->getFirstItem();
        if ($firstItem && ($firstItem instanceof Xcom_Mmp_Model_Policy)) {
            ...
            $gridIds = $this->getParentBlock()->getCollection()
                ->addStatusFilter($allowedStatus)
                ->getAllIds();
            if(!empty($gridIds)) {
                return join(",", $gridIds);
            }
